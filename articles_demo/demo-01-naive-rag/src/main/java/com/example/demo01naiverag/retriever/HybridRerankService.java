...
public class HybridRerankService {
    ...
    public List<TextSegment> searchAndRerank(String query, int topK) {
        System.out.println("1. 执行向量召回 (粗筛 " + maxResults + " 条记录)...");
        ...
        System.out.println("2. 执行 Cross-Encoder 重排序...");
    ...
    private double[] callDashScopeRerankAPI(String query, List<TextSegment> segments) {
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                System.err.println("重排序 API 调用失败, 状态码: " + response.statusCode() + ", 响应: " + response.body());
                return scores;
            }
            ...
            System.err.println("调用大模型重排序 API 时发生异常: " + e.getMessage());
        }
